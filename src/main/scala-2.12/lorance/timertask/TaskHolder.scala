...
class TaskHolder {
  ...
  def ready(newTask: Task): (Boolean, Option[Task]) = {
    readyUnsafe(newTask, {
      case None =>
        ConsoleLog.log(s"none task under waiter - $newTask", 170, Some("dispatch-ready"))
        true
      case Some(nowTask) if newTask.taskId.timestamp < nowTask.taskId.timestamp =>
        ConsoleLog.log(s"replace older waiting task - $nowTask; the task - $newTask", 170, Some("dispatch-ready"))
        true
      case Some(nowTask) =>
        ConsoleLog.log(s"can't replace older task - $nowTask; the task - $newTask", 170, Some("dispatch-ready"))
        false
    })
  }
  ...
  def readyUnsafe(newTask: Task, predicate: (Option[Task]) => Boolean): (Boolean, Option[Task]) = godLock.synchronized {
    ...
    if(predicate(currentTaskRef)) {
      ...
      sleepTime = Some(newTask.taskId.timestamp - System.currentTimeMillis())

      ConsoleLog.log(s"ready - $newTask; currentTime - ${System.currentTimeMillis()}", 120, Some("dispatch-readyUnsafe"))
  ...
  private object Waiter extends Thread {
    ...
    override def run(): Unit = {
      ConsoleLog.log("Waiter thread begin to run", 200)
      godLock.synchronized {
        while(true) {
          ConsoleLog.log(s"loop - ${System.currentTimeMillis()}; cancel - $canceling", 150)

          sleepTime match {
            case None =>
              initStatus()
              ConsoleLog.log(s"Waiter sleep - $sleepTime", 150)
              godLock.wait()
              ConsoleLog.log(s"Waiter awake - ${System.currentTimeMillis()}", 150)
            case Some(delay) =>
              if (!canceling) { //canceling handle execute action and wait.
                if (delay > 0L) {
                  ConsoleLog.log(s"sleep with delay Time - $sleepTime", 150, Some("Waiter"))
                  godLock.wait(delay)
                  ConsoleLog.log(s"awake with delay Time - ${System.currentTimeMillis()}", 150, Some("Waiter"))
                }
                ...
                var tempTask: Option[Task] = None // use the temp ref because do initStatus will lose the task
                  ...
                  ConsoleLog.log("executed action - ", 150, aim = Some("Waiter"))
                  initStatus()
                ...
                ConsoleLog.log(s"cancel waiter - ${System.currentTimeMillis()}", 150, aim = Some("Waiter"))
                canceling = false
              }
          }
        }
